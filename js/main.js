$(function () {
  ...
  function initVisualMotion() {
    const $visualWrap = $('#visualWrap');
    const $visualBox = $visualWrap.find('.visualBox');

    if (!$visualWrap.length || !$visualBox.length) {
      return;
    }

    const $popupContentBox = $('#popupContentBox');
    const $proposalDownloadBtn = $('.proposalDownloadBtn');
    ...
    $visualWrap.removeClass('step02');
    $popupContentBox.removeClass('active');
    $proposalDownloadBtn.removeClass('active');

    /* 초기 상태가 먼저 그려지도록 강제 계산 */
    void $visualBox.get(0).offsetHeight;

    /* 초기 화면 모션 실행 */
    setTimeout(function () {
      $visualBox.addClass('active');
      $popupContentBox.addClass('active');
      $proposalDownloadBtn.addClass('active');
    }, 150);
    ...
    /* 기존 비주얼 트리거 중복 방지 */
    const previousVisualTrigger =
      ScrollTrigger.getById(
        'visual-motion'
      );

    if (previousVisualTrigger) {
      previousVisualTrigger.kill();
    }

    /* 비주얼 스크롤 모션 */
    ScrollTrigger.create({
      id: 'visual-motion',
      trigger: $visualWrap.get(0),
      start: 'top top',
      ...
      onUpdate: function (self) {
        updateVisualStep(self.progress);
      },
  ...
  function initProjectMotion() {
    ...
    const $projectContainer =
      $projectWrap.find('.container');

    const $projectBox =
      $projectWrap.find('.projectBox');

    const $projectList =
      $projectWrap.find('.projectList');
    ...
    function getProjectListWidth() {
      return $projectList.get(0).clientWidth;
    }
    ...
    projectMedia.add(
      ...
      function () {
        ...
        const previousProjectTrigger =
          ScrollTrigger.getById(
            'project-motion'
          );

        if (previousProjectTrigger) {
          previousProjectTrigger.kill();
        }
  ...
  function initCompanyMotion() {
    ...
    function startCompanyCount() {
      ...
      $companyNumbers.each(function () {
        const $number = $(this);

        const target = Number(
          $number.attr('data-target')
        );
